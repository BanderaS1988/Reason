...
export default async function handler(req, res) {
  ...
  // ── LÁJK POST ──────────────────────────────────────────────────────────────
  // A böngésző fetch() hívja: POST /api/cikk/[id]?action=like
  if (req.method === 'POST' && req.query.action === 'like') {
    try {
      // Növeljük a like_count-ot 1-gyel RPC nélkül: először lekérjük, majd PATCH
      ...
      if (!patchR.ok) {
        res.status(500).json({ error: 'like_update_failed' });
        return;
      }
  ...
  const [artRes, commentsRes] = await Promise.all([
    ...
    fetch(
      `${SB_URL}/rest/v1/comments?article_id=eq.${id}&order=created_at.asc`,
      { headers: SB_HEADERS }
    )
  ]);
...
<body>
  ...
  <script>
    ...
    // ── LÁJK ────────────────────────────────────────────────────────────────
    // localStorage-ban tároljuk hogy lájkolta-e már (dupla lájk ellen)
    const LIKE_KEY = 'reason_liked_' + ARTICLE_ID;
    const btn = document.getElementById('likeBtn');
    const countEl = document.getElementById('likeCount');

    // Oldalbetöltéskor visszaállítjuk az állapotot
    if (localStorage.getItem(LIKE_KEY)) {
      btn.classList.add('liked');
    }

    async function handleLike() {
      // Ha már lájkolta, nem csinálunk semmit
      ...
      try {
        const r = await fetch(ARTICLE_URL + '?action=like', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' }
        });
        if (!r.ok) throw new Error('server_error');
        const data = await r.json();
        // Szerveroldali értékkel szinkronizálunk
        countEl.textContent = data.like_count;
      } catch (e) {
        // Rollback ha a szerver hibát adott
        countEl.textContent = prev;
        btn.classList.remove('liked');
        localStorage.removeItem(LIKE_KEY);
        console.error('[Like] Hiba:', e);
      }
    }
    ...
    async function handleShare() {
      ...
      if (navigator.share) {
        try {
          await navigator.share({ title: ARTICLE_TITLE, url: ARTICLE_URL });
          // Ha sikerült, nem kell üzenet – a rendszer dialógusa visszajelzett
        } catch (e) {
          // AbortError: felhasználó bezárta a dialógust – ez nem hiba
          if (e.name !== 'AbortError') {
            console.error('[Share] Hiba:', e);
          }
        }
      } else {
        // Fallback: vágólap
        try {
          ...
          // Régi böngésző vagy nem HTTPS: execCommand fallback
    ...
    async function submitComment() {
      ...
      try {
        ...
        msg.textContent = 'Hozzászólás elküldve!';
        document.getElementById('cBody').value = '';
        document.getElementById('cAuthor').value = '';

        // Oldal újratöltése 1.2 mp után hogy az új komment megjelenjen
        setTimeout(() => location.reload(), 1200);
      } catch (e) {
        msg.style.color = '#c8102e';
        msg.textContent = 'Hiba történt, próbáld újra.';
        console.error('[Comment] Hiba:', e);
      }
    }
  </script>
</body>
  ...
  // like POST után nem cache-elünk, GET-nél 60 mp CDN cache
